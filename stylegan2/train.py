...
def requires_grad_D(D, flag=True, finetune_loc=-1):
    for param in D.conv_blocks[finetune_loc:].parameters():
        param.requires_grad = flag

    for param in D.conv_blocks[finetune_loc:].parameters():
        param.requires_grad = flag

    for param in D.from_data_layers.parameters():
        param.requires_grad = flag

    for param in D.dense.parameters():
        param.requires_grad = flag

def requires_grad_G(G, flag=True, finetune_loc=-1):
    # xxx: G_mapping ???

    for param in G.G_synthesis.conv_blocks[finetune_loc:].parameters():
        param.requires_grad = flag

    for param in G.G_synthesis.to_data_layers.parameters():
        param.requires_grad = flag

    for param in G.G_synthesis.upsample.parameters():
        param.requires_grad = flag
...
class Trainer:
    ...
    def train(self, iterations, callbacks=None, verbose=True):
        ...
        for _ in range(iterations):
            ...
            # Re-enable gradients for while training G
            if self.freezeG < 0:
            ...
            for _ in range(self.G_iter):
                ...
                for i in range(self.subdivisions):
                    ...
                    loss, _ = self.G_loss(
                        ...
                        augment=augment if self.augmentopt["augment"] else None,
                        ada_augment=ada_augment, ada_aug_p=ada_aug_p, ada_aug_step=ada_aug_step 
                    )
                    G_loss += self._backward(loss, self.G_opt)

                #if G_reg and self.freezeG < 0:
                if G_reg:
            ...
            # Re-enable gradients for D while training D
            if self.freezeD < 0:
            ...
            for _ in range(self.D_iter):
                ...
                for i in range(self.subdivisions):
                    ...
                    loss, _, real_scores = self.D_loss(
                        G=self.G,
                        D=self.D,
                        latents=latents,
                        latent_labels=latent_labels,
                        ...
                        augment=augment if self.augmentopt["augment"] else None,
                        ada_augment=ada_augment, ada_aug_p=ada_aug_p, ada_aug_step=ada_aug_step, device=self.device
                    )
                    D_loss += self._backward(loss, self.D_opt)

                if augment is not None and self.augmentopt["augment_p"] == 0:
                    ada_augment += torch.tensor(
                        (torch.sign(real_scores).sum().item(), real_scores.shape[0]), device=self.device
                    )
